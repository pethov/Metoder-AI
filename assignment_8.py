...
def feed_forward(x, hidden_W, hidden_b, out_W, out_b):
    ...
    hidden_activations = sigmoid(hidden_activations)
...
def backward(y_hat, y, hidden_activations, x, out_W):
    ...
    dL_dout_linear = dL_dyhat * d_linear(y_hat)
    # gradients wrt output weights
    d_L_d_ow = hidden_activations.T @ dL_dout_linear  # Size (2,1)
    # gradients wrt output bias
    d_L_d_ob = np.sum(dL_dout_linear, axis=0)  # Size (1,)

    dL_dhidden= dL_dout_linear @ out_W.T
    dL_dhidden_linear = dL_dhidden * d_sigmoid(hidden_activations)
    # gradients wrt hidden weights
    d_L_d_hw = x.T @ dL_dhidden_linear  # Size (2,2)
    # gradients wrt hidden bias
    d_L_d_hb = np.sum(dL_dhidden_linear, axis=0)  # Size (2,)
